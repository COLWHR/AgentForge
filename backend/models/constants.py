from enum import Enum

class ResponseCode(int, Enum):
    SUCCESS = 0
    
    # 1000: General & Validation Errors
    INTERNAL_ERROR = 1000
    VALIDATION_ERROR = 1001
    NOT_FOUND = 1002
    DATABASE_ERROR = 1003
    
    # 2000: Model Gateway Errors
    MODEL_ERROR = 2000
    MODEL_TIMEOUT = 2001
    
    # 3000: Tool Runtime Errors
    TOOL_ERROR = 3000
    
    # 4000: Sandbox Errors
    SANDBOX_ERROR = 4000
    
    # 5000: Quota & Auth Errors
    AUTH_REQUIRED = 5000
    TOKEN_INVALID = 5001
    TOKEN_EXPIRED = 5002
    PERMISSION_DENIED = 5003
    TEAM_FORBIDDEN = 5004
    QUOTA_EXCEEDED = 5005
    RATE_LIMIT_EXCEEDED = 5006

    # 5100: Account & Email Errors
    EMAIL_ALREADY_REGISTERED = 5100
    EMAIL_SEND_RATE_LIMITED = 5101
    EMAIL_PROVIDER_NOT_CONFIGURED = 5102
    EMAIL_SEND_FAILED = 5103
    VERIFICATION_CODE_INVALID = 5104
    VERIFICATION_CODE_EXPIRED = 5105
    VERIFICATION_CODE_ATTEMPTS_EXCEEDED = 5106
    REGISTRATION_TOKEN_INVALID = 5107
    REGISTRATION_TOKEN_EXPIRED = 5108
    PASSWORD_CONFIRM_MISMATCH = 5109
    ACCOUNT_DISABLED = 5110
    
    # Engine specific (if needed, map to 1000 or similar)
    ENGINE_ERROR = 1010

class ExecutionState(str, Enum):
    INIT = "INIT"
    THINKING = "THINKING"
    ACTING = "ACTING"
    OBSERVING = "OBSERVING"
    FINISHED = "FINISHED"
    TERMINATED = "TERMINATED"

class TerminationReason(str, Enum):
    SUCCESS = "SUCCESS"
    USER_STOPPED = "USER_STOPPED"
    MAX_STEPS_REACHED = "MAX_STEPS_REACHED"
    MODEL_OUTPUT_TRUNCATED = "MODEL_OUTPUT_TRUNCATED"
    CONTEXT_BUDGET_EXCEEDED = "CONTEXT_BUDGET_EXCEEDED"
    FAILED = "FAILED"

class ActionType(str, Enum):
    TOOL_CALL = "tool_call"
    FINISH = "finish"
    ERROR = "error"

class QuotaStatus(str, Enum):
    ACTIVE = "ACTIVE"
    EXHAUSTED = "EXHAUSTED"
    RATE_LIMITED = "RATE_LIMITED"


class ExecutionStatus(str, Enum):
    PENDING = "PENDING"
    RUNNING = "RUNNING"
    SUCCEEDED = "SUCCEEDED"
    FAILED = "FAILED"
    TERMINATED = "TERMINATED"


class ArcErrorCode(str, Enum):
    MISSING_API_KEY = "MISSING_API_KEY"
    INVALID_ENDPOINT = "INVALID_ENDPOINT"
    AUTH_FAILED = "AUTH_FAILED"
    MODEL_NOT_FOUND = "MODEL_NOT_FOUND"
    INVALID_MODEL_REQUEST = "INVALID_MODEL_REQUEST"
    MODEL_CAPABILITY_MISMATCH = "MODEL_CAPABILITY_MISMATCH"
    INVALID_TOOL_CALL = "INVALID_TOOL_CALL"
    PROVIDER_RATE_LIMITED = "PROVIDER_RATE_LIMITED"
    MODEL_OUTPUT_TRUNCATED = "MODEL_OUTPUT_TRUNCATED"
    CONTEXT_BUDGET_EXCEEDED = "CONTEXT_BUDGET_EXCEEDED"
    NETWORK_ERROR = "NETWORK_ERROR"
    KNOWLEDGE_REQUIRED_BUT_NOT_FOUND = "KNOWLEDGE_REQUIRED_BUT_NOT_FOUND"
    KNOWLEDGE_RETRIEVAL_FAILED = "KNOWLEDGE_RETRIEVAL_FAILED"
    TOOL_BLOCKED_BY_POLICY = "TOOL_BLOCKED_BY_POLICY"
    TOOL_CONFIRMATION_REQUIRED = "TOOL_CONFIRMATION_REQUIRED"
    TOOL_NOT_ALLOWED_FOR_INTENT = "TOOL_NOT_ALLOWED_FOR_INTENT"
    FINAL_ANSWER_REJECTED_BY_POLICY = "FINAL_ANSWER_REJECTED_BY_POLICY"
    INTENT_CLASSIFICATION_FAILED = "INTENT_CLASSIFICATION_FAILED"
